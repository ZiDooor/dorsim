...
class TableauSim:
    ...
    def _collapse_random_z(self, q: int) -> None:
        row = self.n + q
    ...
    def run(self) -> "TableauSim":
        for op in self.circuit.without_noise().operations:
            if op.name in CLIFFORD_GATES:
                self.append_gate_after_circuit(op)
            elif op.name == "M":
                self.measure_z(op.targets[0], record=True)
            elif op.name == "R":
                self.reset_z(op.targets[0])
        return self
